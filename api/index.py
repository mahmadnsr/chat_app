...
def send():
    ...
    data = request.json
    sender, receiver, msg = session["user"], data.get("to"), data.get("msg")
...
def inbox(chat_with):
    if "user" not in session: return {"error": "Login required"}, 401
    return jsonify(storage.get_messages_between(session["user"], chat_with))
...
def conversations():
    ...
    data = [{"user": u, "unread": storage.get_unread_count(user, u), 
